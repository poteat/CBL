	...
	public:
		...
		void drawRenderGrid(std::string file_name)
		{
			float min_dist_limit = 1.0; // Angstroms
										...
										// Refine voxel projections with setback disabled
			refineProjection(0);
		...
		void drawProjections(std::string file_name)
		{
			writePointsToPDB(file_name, point_projections);
		}
		...
		void refineProjection(float setback)
		{
		...
		float foldednessScore(const Eigen::Matrix3Xf &control)
		{
			...
			//return max_foldedness * max_foldedness;
		...
		float calcSurfaceScore(const Eigen::Matrix3Xf &control)
		{
			...
			// std::cout << "Foldedness: " << foldednessScore(control) << std::endl;

			return sum; // + foldednessScore(control)*100;
		}
		...
		void optimizeSurface()
		{
			...
			control = localBinaryOptimize(control, 1.0, scoring_func, .1);
		...
		Eigen::Matrix3Xf localBinaryOptimize(const Eigen::Matrix3Xf &initial_values, float initial_delta, const std::function<float(const Eigen::Matrix3Xf&)> &scoring_function, const float tolerance)
		{
